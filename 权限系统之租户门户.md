...
但既然新的token被创建了出来，说明原token的有效时间已不足20分钟了。因此我们建议当你的应用程序收到每一个token时，都将它存下用于之后的对IoT的API的访问，而不需要去判断响应头中的token是否是新的。  
因为如果token离过期时间还早，IoT返回给你的原token还可以为你提供相当长的时间段内的访问许可。  
而如果你使用的token快过期了，IoT也会为你生成新的可以提供长时间访问的token。总之，让IoT为你进行是否使用新token的判断，而你的应用程序只管去存最后一次请求返回的token即可。
